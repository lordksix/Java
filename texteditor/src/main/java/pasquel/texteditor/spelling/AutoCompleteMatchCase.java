...
public class AutoCompleteMatchCase implements  Dictionary, AutoComplete {
    ...
    public List<String> predictCompletions(String prefix, int numCompletions) {
		...
		if(temp.endsWord()){
			if(flag) result.add(temp.getText().substring(0, 1).toUpperCase()+temp.getText().substring(1));
			else result.add(temp.getText());
		...
		while(count<numCompletions){
			for (TrieNode trieNode : parNode) {
				if(trieNode.endsWord()){
					if(flag) result.add(trieNode.getText().substring(0, 1).toUpperCase()+trieNode.getText().substring(1));
					else result.add(trieNode.getText());
					count++;
				}
				if(count==numCompletions) return result;
			}
