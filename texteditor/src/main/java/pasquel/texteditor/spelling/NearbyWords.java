...
public class NearbyWords implements SpellingSuggest {
	...
	public List<String> suggestions(String word, int numSuggestions) {
		...
		while(!queue.isEmpty() && retList.size()<numSuggestions){
			...
			List<String> currList = distanceOne(curr, false);
			for (String string : currList) {
				if(!visited.contains(string)){
					visited.add(string);
					queue.add(string);
					permut++;
					if(dict.isWord(string)) retList.add(string);
				}
				if(retList.size()==numSuggestions || permut>THRESHOLD) return retList;
			}
		}		
		return retList;
	}	
   ...
   public static void main(String[] args) {
	   ...
	   AutoCompleteDictionaryTrie d = new AutoCompleteDictionaryTrie();
