...
public class AutoSpellingTextArea extends StyledTextArea<String,Boolean> {
	...
	private pasquel.texteditor.spelling.AutoComplete ac;
	private pasquel.texteditor.spelling.Dictionary dic;
	private pasquel.texteditor.spelling.SpellingSuggest ss;
...
/* 	public AutoSpellingTextArea(String initialParagraphStyle, BiConsumer<TextFlow, String> applyParagraphStyle,
				Boolean initialTextStyle, BiConsumer<? super TextExt, Boolean> applyStyle, AutoComplete ac, Dictionary dic,
				SpellingSuggest ss) {
	...
	public AutoSpellingTextArea(AutoComplete ac, SpellingSuggest ss, Dictionary dic) {
		super("", (t, s) -> {},true, (textNode, correct) -> {
			// define boolean Text node style
			if (!correct) {
				textNode.setUnderline(true);
				textNode.setBackgroundColor(Color.TOMATO);
			}
		});
	...
	private void showSuggestions(String word, MouseEvent click) {
		...
		item.setOnAction(new EventHandler<ActionEvent>() {
			...
			public void handle(ActionEvent actionEvent) {
				...
				dic.addWord(word);
				((pasquel.texteditor.spelling.Dictionary) ac).addWord(word);
